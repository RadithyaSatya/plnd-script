...
try:
    strobe = serial.Serial('/dev/ttyAMA3', 115200, timeout=1) 
    print("Strobe connected on /dev/ttyAMA3")
    time.sleep(1)
except Exception as e:
    print(f"Cannot open /dev/ttyAMA3 → {e}")
    strobe = None
...
try:
    master = mavutil.mavlink_connection('udp:127.0.0.1:14551', timeout=0.1)
except Exception as e:
    print(f"Error: Could not connect to UDP endpoint udp:127.0.0.1:14551: {e}") 
    signal_handler(signal.SIGINT, None)
